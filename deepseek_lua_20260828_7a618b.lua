...
-- Модифицирован: добавлен умный фарм по этапам и телепорты на команды
...
-- 1. УМНЫЙ ФАРМ ПО ЭТАПАМ (с выбором количества этапов)
local smartFarmEnabled = false
local stagesToFarm = 2 -- количество этапов до ресета (по умолчанию 2)
...
local function smartFarmCycle()
    ...
    -- Если текущий индекс превысил заданное количество этапов, сбрасываем на 1
...
superFarmTab:CreateParagraph({
    ...
    Content = "Проходит N этапов, затем сбрасывается. За каждый этап даётся 8 золота."
})
...
-- 2. ТЕЛЕПОРТЫ НА КОМАНДЫ (цвета)
local teleportTab = Window:CreateTab("Телепорты","rewind")

-- Функция поиска базы по цвету команды
local function getTeamBaseByColor(colorName)
    ...
    for _,v in pairs(workspace:GetChildren()) do
        if v:FindFirstChild("TeamColor") and v.TeamColor.Value then
            if v.TeamColor.Value == targetColor then
                return v
            end
        end
    end
...
-- Список локаций (добавляем команды)
local locations = {
    "Старт (зона 1)",
    "Финиш (сундук)",
    "Магазин",
...
teleportTab:CreateButton({
    ...
    Callback = function()
        ...
        elseif selectedLocation == "Магазин" then
            -- Примерные координаты (замени на реальные)
            targetPos = CFrame.new(0, 5, 0)
        elseif selectedLocation == "Зона крафта" then
            targetPos = CFrame.new(0, 5, 0)
        elseif selectedLocation == "Арена" then
            targetPos = CFrame.new(0, 5, 0)
        elseif selectedLocation == "Красная команда" or selectedLocation == "Синяя команда" or 
               selectedLocation == "Зелёная команда" or selectedLocation == "Белая команда" or 
               selectedLocation == "Чёрная команда" then
            local colorName = string.match(selectedLocation, "^(%S+)") -- извлекаем цвет
            local base = getTeamBaseByColor(colorName)
            if base then
                targetPos = base.CFrame + Vector3.new(0, 5, 0) -- центрируем
            else
                Rayfield:Notify({
                    Title = "Ошибка",
                    Content = "База команды " .. colorName .. " не найдена",
                    Duration = 5,
                    Image = "alert-triangle"
                })
            end
...
teleportTab:CreateInput({
    ...
    PlaceholderText = "0",
    Callback = function(text)
        customCoords.X = tonumber(text) or 0
    end,
})
teleportTab:CreateInput({
    Name = "Y",
    PlaceholderText = "0",
    Callback = function(text)
        customCoords.Y = tonumber(text) or 0
    end,
})
teleportTab:CreateInput({
    Name = "Z",
    PlaceholderText = "0",
    Callback = function(text)
        customCoords.Z = tonumber(text) or 0
    end,
})

-- 3. СТРОИТЕЛЬСТВО ПО СХЕМЕ (JSON)
...
local function buildFromJSON(jsonText)
    ...
    Rayfield:Notify({
        Title = "Готово",
        Content = "Построено " .. placed .. " блоков",
        Duration = 5,
        Image = "check-circle"
    })
    pastePercent = 0
...
buildFromSchemeTab:CreateButton({
    ...
    Callback = function()
        if _G.schemeJSON and #_G.schemeJSON > 0 then
            ...
            Rayfield:Notify({
                Title = "Ошибка",
                Content = "Сначала введите JSON-схему",
                Duration = 5,
                Image = "alert-triangle"
            })
        end
...
buildFromSchemeTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            ...
            Content = "Вставьте свой JSON поверх",
            Duration = 3,
            Image = "info"
        })
    end,
...
-- ======================== ОСТАЛЬНОЙ КОД (автофарм до сундука, анти-AFK) ========================
task.spawn(function()
    while true do
        task.wait()
        if autofarm then
